...
export default function CreateForm() {
  ...
  async function handleSave() {
    ...
    if (fields.length === 0) {
      Alert.alert('No fields', 'Please add at least one field to the form.');
      return;
    }
    ...
    if (templateError || !template) {
      Alert.alert('Error', templateError?.message ?? 'Failed to create form');
      setLoading(false);
      return;
    }
    ...
    if (fieldsError) {
      ...
      Alert.alert('Form created', `"${name}" is now available for employees.`, [
    ...
    <>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={[styles.headerTitle, { color: theme.text }]}>Create Form</Text>
          <View style={{ width: 24 }} />
        </View>
          ...
          {fields.length === 0 ? (
            <View style={[styles.emptyFields, { borderColor: theme.border }]}>
              <Text style={[styles.emptyFieldsText, { color: theme.muted }]}>
                No fields yet — tap Add Field to start building your form
              </Text>
            </View>
          ) : (
